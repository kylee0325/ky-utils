...
export interface Helper<RMK extends ResultMapKey, RootType> {
    <K extends string>(keys: K[]): HelperReturnType<RMK, K, RootType>;

    <K extends string, N extends string>(namespace: N, keys: K[]): HelperReturnType<
        RMK,
        K,
        N extends keyof RootType ? RootType[N] : {}
    >;
...
export function createVuexHelpers<RootState, RootGetters, RootMutations, RootActions>() {
    return {
