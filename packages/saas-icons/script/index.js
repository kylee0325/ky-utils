...
// const svgfilesPath = "https://at.alicdn.com/t/font_1825326_z3so6t1vn1.js";
const svgfilesPath = "https://at.alicdn.com/t/font_2698958_khzqu2owpk9.js";
...
function downloadFile(uri, filename, callback) {
  ...
  const stream = fs.createWriteStream(path.join(iconsDir, filename));
...
function svgformat(filePath) {
  fs.readFile(path.join(iconsDir, filename), "utf8", (error, data) => {
    if (error) {
      throw error;
    } else {
    ...
    ${svgTmp}
      ...
      fs.writeFileSync(path.join(iconsDir, "iconfont.svg.js"), svgStr, "utf8");
      // 删除下载的文件
      fs.unlink(path.join(iconsDir, filename), (err) => {
        if (err) throw err;
      });
    }
  });
